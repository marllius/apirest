...
# git config
GIT_REPO_OWNER="marllius"
GIT_REPO_NAME="Revolut"
GIT_BRANCH="main"
GIT_ACCESS_TOKEN=":"
GIT_CLONE_URL=""
...
echo $(date -u) "Strating deploy script" 1&>/var/log/app/app.log
...
function setup_venv() {
    ...
    python -m virtualenv -p $VM_PY_PATH $VM_HOME_DIR/venv
...
function run_tests() {
    ...
    if [[ -d $test_folder ]]; then
        echo ====== start docker ========
        sudo docker run -d --name postgres -p 5432:5432 -e POSTGRES_PASSWORD=postgres postgres:13
        echo ====== Installing initialize database script ========
        myip=$(hostname -I | awk '{print $1}')
        sudo docker run -it --rm -e PGPASSWORD=postgres postgres:13 psql -h $myip -U postgres -f ../api_test.sql
